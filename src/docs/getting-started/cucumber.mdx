...
→ Learn more: [Cucumber configuration](/handbook/test-runners/cucumber/) | [Serenity BDD reporter setup](/handbook/reporting/serenity-bdd-reporter/)
...
Feature: Website
  ...
  Scenario: Serenity/JS website should have a descriptive title
    When Serena navigates to the Serenity/JS website
...
When('{actor} navigates to the Serenity/JS website', async (actor) => {
    await actor.attemptsTo(
        Navigate.to('https://serenity-js.org/'),
    );
});

Then('{pronoun} should see that the page title includes {string}', async (actor, expectedTitle) => {
    await actor.attemptsTo(
        Ensure.that(
...
- **`{actor}` and `{pronoun}`** are Serenity/JS [Cucumber expression parameters](/api/cucumber/) that automatically resolve to the correct actor instance.
...
- **[Why Serenity/JS?](/getting-started/)** — see how Serenity/JS transforms flat test scripts into structured, maintainable code
...
If you don't need Gherkin syntax, Serenity/JS works directly with Playwright Test for a simpler setup. See [Serenity/JS with Playwright](/getting-started/playwright/).
:::
