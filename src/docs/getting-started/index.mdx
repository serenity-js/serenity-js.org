...
To show you what that looks like in practice, this page walks through the **same scenario** at four levels of Serenity/JS adoption — from a flat test script to structured, reusable code with rich reporting.
...
- **Reports show only pass/fail** — when something breaks, you're reading raw locators to figure out what the test was even verifying
...
The following scenario is implemented at four levels of Serenity/JS adoption — from vanilla Playwright through to the full Screenplay Pattern. Each level builds on the previous one, and you can stop wherever it makes sense for your project.
...
**What breaks down at scale:** No reuse, no structured reporting, no abstraction. Every test that needs login duplicates those selectors. When one breaks, you're reading raw locators to figure out what it was even testing.
...
→ Learn more: [Using npm-failsafe](https://www.npmjs.com/package/npm-failsafe)
