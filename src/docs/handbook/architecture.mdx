...
Setting up a Serenity/JS test runner adapter and reporting services is the first thing most developers do when introducing Serenity/JS to an existing codebase.
:::
...
**Serenity/JS Project Templates** and **Serenity/JS GitHub Codespaces** come with all the appropriate Serenity/JS modules
and lower-level integration and test tools already configured, so you can start automating your tests straight away!

Learn more about faster ways to get started using [Serenity/JS Project Templates](/getting-started/project-templates/).
:::
