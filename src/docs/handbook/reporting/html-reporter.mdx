...
### Cucumber, Mocha, and Jasmine

When using Cucumber, Mocha, or Jasmine directly (without WebdriverIO), configure crew members programmatically:

```ts title="support/serenity.config.ts"
...
For Cucumber, call this in a `BeforeAll` hook. For Mocha and Jasmine, require the file via your test runner's configuration.

Learn more about using Serenity/JS with [Cucumber](/handbook/test-runners/cucumber/), [Mocha](/handbook/test-runners/mocha/), or [Jasmine](/handbook/test-runners/jasmine/).
...
The `serve` command defaults to `./reports/serenity-js` — if your `outputDirectory` matches the default, you don't need to specify `--dir`.
