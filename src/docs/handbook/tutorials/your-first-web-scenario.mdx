...
generated under `target/site/serenity`, and served on port `8080` in GitHub Codespace:
...
under `playwright-report` and served on port `8181` in GitHub Codespace:
...
the `Photographer` differently for different environments, browsers, or test suites, while keeping the `SerenityBDDReporter`
and `ArtifactArchiver` configuration the same across all of them.
:::
...
and the `ArtifactArchiver` and `SerenityBDDReporter` will automatically add your screenshot to the report:
