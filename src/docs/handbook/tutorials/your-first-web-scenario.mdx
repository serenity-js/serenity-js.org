...
To keep things simple, we'll use a [**GitHub Codespace**](/getting-started/project-templates/#serenityjs-codespaces) to work with Serenity/JS in your web browser, so there's **no need to install anything** on your computer.
...
👉 [Start a GitHub Codespace](https://github.com/codespaces/new?repo=serenity-js/serenity-js-playwright-test-template&ref=main)
...
All [Serenity/JS project templates](/getting-started/project-templates/), such as the one we'll use in this chapter,
support [GitHub Codespaces](/getting-started/project-templates/#serenityjs-codespaces) and are configured to
...
Next, 👉 [create your GitHub Codespace](https://github.com/codespaces/new?repo=serenity-js/serenity-js-playwright-test-template&ref=main).
...
such as Selenium, Playwright and WebdriverIO. Once you know your way around your first [Serenity/JS project template](/getting-started/project-templates/),
...
you've just opened in your GitHub Codespace includes several example test scenarios
...
Once you've inspected the [`spec/recording_items.spec.ts`](https://github.com/serenity-js/serenity-js-playwright-test-template/blob/main/spec/recording_items.spec.ts),
...
1. Open [`spec/recording_items.spec.ts`](https://github.com/serenity-js/serenity-js-playwright-test-template/blob/main/spec/recording_items.spec.ts) and run test scenarios such as `it('should clear text input field when an item is added')` individually using the play/check mark icon next to the `it` block. Make sure they're passing.
...
First, add the below code snippet to your existing test suite in [`spec/recording_items.spec.ts`](https://github.com/serenity-js/serenity-js-playwright-test-template/blob/main/spec/recording_items.spec.ts)
...
<Figure
    caption='Adding a new Serenity/JS test scenario using Visual Studio Code in GitHub Codespace'
...
[Serenity/JS + Playwright Test GitHub Codespace](https://github.com/serenity-js/serenity-js-playwright-test-template)
I have prepared for you comes equipped with [VNC](https://en.wikipedia.org/wiki/Virtual_Network_Computing),
so you can see the browser as well.
...
- In the "Ports" panel, open VNC running on port `6080` in "Preview" mode (this step applies only to GitHub Codespaces)
    - If your "Ports" panel is empty, click the bell in the lower right corner and click the appropriate "Open preview" button.
- Run the test!

<Figure
    caption='Viewing the browser window while running Serenity/JS test scenarios in Visual Studio Code in GitHub Codespace'
...
The Serenity/JS activity-based code reuse model also means that as you evolve the vocabulary of your domain-specific tasks
...
and modify the task called `startWithAnEmptyList` by adding the interaction to `Debug` and set a breakpoint on the line indicated by the comment:
...
Serenity/JS models the act of information retrieval as [questions](/handbook/design/screenplay-pattern#questions)—an
actor answers a question to obtain some information. Since questions represent a **way for actor to obtain the information**,
...
you'll notice that the actor has resolved the question returned by `itemNames()` and
...
The Serenity/JS [Playwright Test module](/handbook/test-runners/playwright-test) enables you to leverage the
...
- use the interaction to [`Debug`](/api/core/class/Debug) to pause the scenario at a breakpoint,
...
1. Read the guide on [Serenity/JS debugging](/handbook/design/debugging/#exploring-playwright-locators) and experiment
...
The [Serenity/JS HTML Reporter](/handbook/reporting/html-reporter/) produces a self-contained report with
trends, flaky test detection, and an interactive dashboard.
...
you've been using in this tutorial is already set up to produce both the Serenity/JS HTML and Playwright Test reports.
...
When the test suite is finished, you'll find your [Serenity/JS HTML Report](/handbook/reporting/html-reporter/)
generated under `reports/serenity-js`, and served on port `8080` in GitHub Codespace:

<Figure
    caption='Serenity/JS HTML Report — scenario detail with activity tree'
    img={require('@site/static/images/web-testing/your-first-web-scenario/html-reporter-scenario-detail.png')}
    externalLink={'https://serenity-js.github.io/serenity-js-playwright-test-template/serenity-js/'}
/>

[Playwright Test reports](/handbook/test-runners/playwright-test/reporting/) are generated
under `reports/playwright`, and served on port `8181` in GitHub Codespace:
...
Serenity/JS can capture screenshots of the system under test either automatically using the [`Photographer`](/api/web/class/Photographer/)
or on demand using the interaction to [`TakeScreenshot`](/api/web/class/TakeScreenshot).
...
and add the [`HtmlReporter`](/api/html-reporter/class/HtmlReporter/) to your reporter process:
...
Use [Playwright Projects](https://playwright.dev/docs/test-projects) to configure
the `Photographer` differently for different environments, browsers, or test suites, while keeping the `HtmlReporter`
configuration the same across all of them.
:::

Automatic screenshots are taken using the active actor's web browser when the desired conditions are met,
as defined by your configured strategy:
- `TakePhotosOfFailures`: takes screenshots of the active actor's browser whenever an assertion fails or an error occurs; this is the best strategy in most scenarios as it reduces the size of the HTML report and helps you quickly identify the cause of any failure
- `TakePhotosOfInteractions`: takes screenshots of the system under test after each actor's interaction; useful for producing comprehensive living documentation
- `TakePhotosBeforeAndAfterInteractions`: takes screenshots of the system under test before and after each actor's interaction; helpful in debugging and troubleshooting

:::tip Multi-actor scenarios
Serenity/JS actor-centric model means that in multi-actor scenarios you'll get screenshots from each actor's browser,
even when the actors [use different browsers](https://github.com/serenity-js/serenity-js-playwright-test-template/blob/main/spec/multi-actor_scenarios.spec.ts).
:::

#### On-demand screenshots

To take a screenshot on demand, use the interaction to [`TakeScreenshot`](/api/web/class/TakeScreenshot)
and the `HtmlReporter` will automatically add your screenshot to the report:
...
:::tip Automatic and on-demand screenshots
Note that you can use both the automatic an on-demand strategies simultaneously.
The most common scenario is to configure the `Photographer` to `TakePhotosOfFailures`,
and use `TakeScreenshot` to capture additional screenshots when needed.
:::
