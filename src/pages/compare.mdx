...
→ Learn more: [Dave Farley — Acceptance Testing for Continuous Delivery](https://www.youtube.com/watch?v=JDD5EEJgpHU) | [ATDD course on CD.Training](https://courses.cd.training/courses/atdd-from-stories-to-executable-specifications)
...
import { Cart } from './screenplay/Cart';
...
The Tasks encapsulate the implementation details:

```typescript title="tests/screenplay/Authenticate.ts"
...
export class Authenticate {
    ...
    private static loginButton = () =>
        PageElement.located(By.css('[data-test="login-button"]'))
    ...
    static withCredentials = (username: string, password: string) =>
        Task.where(`#actor logs in as ${ username }`,
            ...
            Enter.theValue(password).into(Authenticate.passwordField()),
            Click.on(Authenticate.loginButton()),
        );
}
```

```typescript title="tests/screenplay/Cart.ts"
import { Task } from '@serenity-js/core';
import { Click, PageElement, By } from '@serenity-js/web';

export class Cart {
    private static addToCartButton = (itemName: string) =>
...
```typescript title="tests/screenplay/Checkout.ts"
import { Task } from '@serenity-js/core';
...
export class Checkout {
    private static checkoutButton = () =>
        PageElement.located(By.css('[data-test="checkout"]'))
            .describedAs('checkout button');

    private static firstNameField = () =>
        PageElement.located(By.css('[data-test="firstName"]'))
            .describedAs('first name field');

    private static lastNameField = () =>
        PageElement.located(By.css('[data-test="lastName"]'))
            .describedAs('last name field');

    private static postalCodeField = () =>
        PageElement.located(By.css('[data-test="postalCode"]'))
            .describedAs('postal code field');

    private static continueButton = () =>
        PageElement.located(By.css('[data-test="continue"]'))
            .describedAs('continue button');

    private static finishButton = () =>
        PageElement.located(By.css('[data-test="finish"]'))
            .describedAs('finish button');

    private static confirmationHeader = () =>
        PageElement.located(By.css('[data-test="complete-header"]'))
            .describedAs('order confirmation heading');

    static completeWith = (info: CheckoutInfo) =>
        Task.where(`#actor completes checkout`,
            Cart.open(),
            Click.on(Checkout.checkoutButton()),
...
**When this makes sense:** 50+ scenarios, multiple contributors, multi-user workflows, or stakeholders who need to understand test coverage.
...
```mermaid
flowchart TD
    A[Do you want to change your existing test code?] -->|No| B[Reporting-Only]
    A -->|Yes, for new tests| C[Are you doing BDD with Gherkin?]
    C -->|No| D[Lean Screenplay]
    C -->|Yes| E[Full Screenplay + BDD]

    B --> F[Start here, grow later]
    D --> G[Best balance of effort vs. value]
    E --> H[Maximum structure and documentation]
```

| | Vanilla | Reporting-Only | Full Screenplay |
|---|---|---|---|
| **Setup** | None | 5 minutes | 1–2 hours to learn |
| **Code changes** | — | Change one import | Restructure into Tasks |
| **Reporting** | Pass/fail + trace | Activity breakdown + screenshots | Hierarchical tasks + living docs |
| **Code reuse** | Copy-paste | Same as vanilla | Composable Tasks |
| **Multi-actor** | Manual | Manual | Built-in |
| **Tool portability** | Locked in | Locked in | Tasks work anywhere |
| **Best for** | Small suites | Better reports now | Growing suites |

:::tip Start with Level 2, grow into Level 3
Most teams get the best ROI by starting with **reporting-only mode** (one import change, zero risk) and gradually introducing Tasks for new tests. You don't have to rewrite anything.
:::
